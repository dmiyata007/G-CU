// Version 2.0.r
// Processing is distributed across core0 and core1 to improve efficiency.
...
// Sensor Numbers
constexpr unsigned char sensors_rows_num = 10;
constexpr unsigned char sensors_columns_num = 11;
...
// WiFi Parameters
const char* host = "esp32s3_armband_v3";
...
// since 2025.4.1
// Variables for core variance
volatile bool dataReceiveFlag = false;
...
void imuTask(void* param){
  ...
  while(true){
    ...
    if (millis() - lastPrintTime > 2000) {
      Serial.print("[IMU Task] Stack HighWaterMark: ");
...
void dataTask(void* param) {
  ...
  while (true) {
    ...
    // ★ 2秒おきにスタック残量を表示
    if (millis() - lastPrintTime > 2000) {
      Serial.print("[DATA Task] Stack HighWaterMark: ");
      Serial.println(uxTaskGetStackHighWaterMark(NULL));
      lastPrintTime = millis();
    }
  }
...
void setup() {
  ...
  data_receiver.attach_ms(1000/device_frequency, onTimerTick);

  //Enable xTask to Core0
  xTaskCreatePinnedToCore(imuTask, "IMU Task", 8192, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(dataTask, "Data Task", 8192, NULL, 1, &dataTaskHandle, 0);
...
void loop() {
  ...
  server.handleClient();
  vTaskNotifyGiveFromISR(dataTaskHandle, NULL);
  ...
  if (working_flag && !led_is_on) {
    neopixelWrite(0, GCU_RGB_BRIGHTNESS, 0);
    led_is_on = true;
  }

  if (ota_mode) {
    ArduinoOTA.handle();  // OTAモードのときだけ呼ぶ
  }
}
