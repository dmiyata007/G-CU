...
void prepareSensorData() {
  ...
  data_p = data; // 開始ポインタ
  if (start_flag)  data_p += 2;
  if (device_num_flag) data_p += 1;
  if (sensors_num_flag) data_p += 1;
  ...
  for (uint8_t i = 0; i < sensors_rows_num; i++) {
    for (uint8_t j = 0; j < sensors_columns_num; j++) {
      ...
      // uint16_t raw = analogReadMilliVolts(analogReadIO[i]);
      uint16_t raw = ( analogRead(analogReadIO[i]) * 3300) / 4095; // ADC値->mv変換（例：12bit -> 4095, 10bit -> 1023）
      ...
      if (normalized_calibration_flag) {
        ...
        uint32_t sensorData = raw;
        memcpy(data_p, &sensorData, sizeof(sensorData));
        data_p += sizeof(sensorData);
      } else {
        uint16_t sensorData = raw;
        memcpy(data_p, &sensorData, sizeof(sensorData));
        data_p += sizeof(sensorData);
      }
  ...
  // 最後に明示的に End flag を書く
  // data[data_num - 2] = 0xA5;
  // data[data_num - 1] = 0xA5;

  if (IMU_flag) {
    if (IMU_chip) {
      memcpy(data_p, &BMI270_BMM150_magn_x, sizeof(float) * 9);
    } else {
      memcpy(data_p, &Omagn, sizeof(Omagn));
      memcpy(data_p, &Ogyro, sizeof(Ogyro));
      memcpy(data_p, &Oaccel, sizeof(Oaccel));
    }
    data_p += 36;
  }

  // バッファポインタ戻す（for UDP送信の再利用）
